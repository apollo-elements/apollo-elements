...
test('hasAllVariables Validates one required variable', function(t) {
  const query = r1;
...
test('hasAllVariables Validates two required variables', function(t) {
  const query = r2;
...
test('hasAllVariables Validates one optional variable', function(t) {
  const query = o1;
  t.equal(hasAllVariables({ query, variables: { one: '1' } }), true, 'With variable');
...
test('hasAllVariables Validates two optional variables', function(t) {
  const query = o2;
...
test('hasAllVariables Validates one required and one optional variable', function(t) {
  const query = r1o1;
  t.equal(hasAllVariables({ query, variables: { one: 'one', two: 'two' } }), true, 'With all variables');
...
test('hasAllVariables Handles weird input', function(t) {
  const query = r1;
