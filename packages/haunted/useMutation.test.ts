...
describe('[haunted] useMutation', function() {
  ...
  describeMutation({
    async setupFunction<T extends MutationElement>(opts: SetupOptions<T>) {
      ...
      const properties: Partial<ApolloMutationElement> =
        (opts?.properties ?? {} as Partial<ApolloMutationElement>);
      ...
      function Mutation(this: MutationElement) {
        const [, { called, loading, data, error }] = useMutation(null, {
          awaitRefetchQueries,
          client,
          context,
          ignoreResults,
